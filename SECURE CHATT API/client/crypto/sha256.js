// sha256.js — Manual SHA-256 Hashing (No Libraries)
...
function sha256(message) {
  ...
  for (let i = 0; i < withPadding.length; i += 64) {
    ...
    const block = new DataView(withPadding.buffer, i, 64);
    for (let t = 0; t < 16; t++) {
    ...
    H = H.map((val, i) => (val + [a, b, c, d, e, f, g, h][i]) >>> 0);
  }

  return H.map(toHex).join('');
}

// Export methods (compatible with other techniques)
export function encrypt(_, plaintext) {
  return sha256(plaintext);
}

export function decrypt() {
  return '❌ SHA-256 is not decryptable';
}
